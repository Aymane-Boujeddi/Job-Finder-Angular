...
export class JobService {
  ...
  searchJobs(keyword: string, location: string, page: number): Observable<JobResponse> {
      ...
      map((response) => {
        ...
        const jobsItems = items.map((item) => {
          ...
          return {
            ...
            salary: salaryArr
              ? `${salaryArr.MinimumRange}$ - ${salaryArr.MaximumRange}$ ${salaryArr.Description}`
              : 'No Salary in this offer',
          };
        });
        return {
          jobs: jobsItems,
